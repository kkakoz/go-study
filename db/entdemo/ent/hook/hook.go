...
import (
	"context"
	"fmt"
	ent2 "learn-go/db/entdemo/ent"
)

// The EducationFunc type is an adapter to allow the use of ordinary
// function as Education mutator.
type EducationFunc func(context.Context, *ent2.EducationMutation) (ent2.Value, error)

// Mutate calls f(ctx, m).
func (f EducationFunc) Mutate(ctx context.Context, m ent2.Mutation) (ent2.Value, error) {
	mv, ok := m.(*ent2.EducationMutation)
	if !ok {
...
type InstitutionFunc func(context.Context, *ent2.InstitutionMutation) (ent2.Value, error)

// Mutate calls f(ctx, m).
func (f InstitutionFunc) Mutate(ctx context.Context, m ent2.Mutation) (ent2.Value, error) {
	mv, ok := m.(*ent2.InstitutionMutation)
	if !ok {
